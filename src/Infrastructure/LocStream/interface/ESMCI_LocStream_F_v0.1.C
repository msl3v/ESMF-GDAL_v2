...
int shapefile_to_parmetis_graph(const char *filename, MPI_Comm comm, 
    ...
    /*==========================================================================
     * DEBUG: Analyze edges BEFORE global ID conversion
     *========================================================================*/
    printf("==========================================\n");
    printf("DEBUG Rank %d: Edges BEFORE global offset (local IDs)\n", rank);
    printf("  edge_count: %lld\n", (long long)edge_count);
    printf("  node_counter (local nodes created): %lld\n", (long long)node_counter);
    printf("  First 5 edges (if available):\n");
    for (idx_t i = 0; i < edge_count && i < 5; i++) {
        printf("    Edge %lld: from=%lld -> to=%lld\n", 
               (long long)i, (long long)edges[i].from, (long long)edges[i].to);
    }
    printf("==========================================\n");
...
//    MPI_Gather(&local_nodes, 1, MPI_INT64_T, node_counts, 1, MPI_INT64_T, 0, comm);
    ...
    /*==========================================================================
     * DEBUG: Analyze edges AFTER global ID conversion - CHECK FOR CROSS-PE EDGES
     *========================================================================*/
    printf("==========================================\n");
    printf("DEBUG Rank %d: Edges AFTER global offset conversion\n", rank);
    printf("  global_offset: %lld\n", (long long)global_offset);
    printf("  My global node range: %lld to %lld\n", 
           (long long)graph->nodedist[rank], (long long)(graph->nodedist[rank+1]-1));
    
    // Count edges by type
    idx_t local_edges = 0;   // Both nodes local
    idx_t boundary_edges_from = 0;  // Source local, dest remote
    idx_t boundary_edges_to = 0;    // Source remote, dest local
    idx_t remote_edges = 0;  // Both nodes remote
    
    for (idx_t i = 0; i < edge_count; i++) {
        int from_local = (edges[i].from >= graph->nodedist[rank] && 
                         edges[i].from < graph->nodedist[rank+1]);
        int to_local = (edges[i].to >= graph->nodedist[rank] && 
                       edges[i].to < graph->nodedist[rank+1]);
        
        if (from_local && to_local) {
            local_edges++;
        } else if (from_local && !to_local) {
            boundary_edges_from++;
        } else if (!from_local && to_local) {
            boundary_edges_to++;
        } else {
    ...
    printf("    Both nodes local: %lld\n", (long long)local_edges);
    printf("    Boundary (src local, dst remote): %lld  <-- CROSS-PE OUTGOING\n", 
           (long long)boundary_edges_from);
    printf("    Boundary (src remote, dst local): %lld  <-- CROSS-PE INCOMING\n", 
           (long long)boundary_edges_to);
    printf("    Both nodes remote: %lld\n", (long long)remote_edges);
    
    // Show sample boundary edges
    if (boundary_edges_from > 0) {
        printf("  Sample OUTGOING boundary edges (src local, dst remote):\n");
        int shown = 0;
        for (idx_t i = 0; i < edge_count && shown < 3; i++) {
            int from_local = (edges[i].from >= graph->nodedist[rank] && 
                             edges[i].from < graph->nodedist[rank+1]);
            int to_local = (edges[i].to >= graph->nodedist[rank] && 
                           edges[i].to < graph->nodedist[rank+1]);
            if (from_local && !to_local) {
                printf("    Edge %lld: from=%lld (local) -> to=%lld (REMOTE)\n",
                       (long long)i, (long long)edges[i].from, (long long)edges[i].to);
                shown++;
            }
        }
    ...
    *node_y = local_node_y;
    ...
    /*==========================================================================
     * DEBUG: CRITICAL BUG LOCATION - Show what edges are being FILTERED
     *========================================================================*/
    printf("==========================================\n");
    printf("DEBUG Rank %d: COUNTING DEGREES (CSR Phase 1)\n", rank);
    printf("  *** THIS IS WHERE CROSS-PE EDGES MAY BE FILTERED ***\n");
    
    idx_t counted_edges = 0;
    idx_t filtered_edges = 0;
    ...
    for (idx_t i = 0; i < edge_count; i++) {
        ...
        if (local_from >= 0 && local_from < local_nodes) {
            ...
            // Show first few counted edges
            if (counted_edges <= 3) {
                printf("  COUNTED Edge %lld: from=%lld (local_from=%lld) -> to=%lld\n",
                       (long long)i, (long long)edges[i].from, 
                       (long long)local_from, (long long)edges[i].to);
            }
        } else {
            filtered_edges++;
            
            // Show first few filtered edges
            if (filtered_edges <= 3) {
                printf("  FILTERED Edge %lld: from=%lld (local_from=%lld OUT OF RANGE) -> to=%lld\n",
                       (long long)i, (long long)edges[i].from, 
                       (long long)local_from, (long long)edges[i].to);
            }
        }
    ...
    printf("  Summary: COUNTED %lld edges, FILTERED %lld edges\n", 
           (long long)counted_edges, (long long)filtered_edges);
    printf("  *** If filtered_edges > 0, those are edges FROM other PEs! ***\n");
    printf("==========================================\n");
    ...
    /*==========================================================================
     * DEBUG: Analyze final CSR structure - VERIFY CROSS-PE EDGES
     *========================================================================*/
    printf("==========================================\n");
    printf("DEBUG Rank %d: FINAL CSR STRUCTURE\n", rank);
    printf("  graph->nnodes: %lld\n", (long long)graph->nnodes);
    printf("  graph->nedges: %lld\n", (long long)graph->nedges);
    
    // Count boundary edges in final CSR
    idx_t boundary_in_csr = 0;
    ...
    printf("  Boundary edges in CSR: %lld\n", (long long)boundary_in_csr);
    
    if (boundary_in_csr == 0) {
        printf("  *** WARNING: NO boundary edges in final CSR! ***\n");
        printf("  *** This means network is disconnected across PEs! ***\n");
    } else {
        printf("  ✓ Good: Cross-PE edges present in CSR\n");
    }
    
    // Show sample adjncy entries
    printf("  First 10 adjncy entries:\n");
    for (idx_t i = 0; i < graph->nedges && i < 10; i++) {
        int is_boundary = (graph->adjncy[i] < graph->nodedist[rank] || 
                          graph->adjncy[i] >= graph->nodedist[rank+1]);
        printf("    adjncy[%lld] = %lld %s\n", 
               (long long)i, (long long)graph->adjncy[i],
               is_boundary ? "(BOUNDARY)" : "(local)");
    }
    printf("==========================================\n");
