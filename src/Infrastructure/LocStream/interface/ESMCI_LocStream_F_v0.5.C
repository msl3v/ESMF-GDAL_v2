...
#define ESMC_METHOD "c_esmc_gdal_shpgetcoords()"
  ...
  /*===========================================================================
   * c_esmc_gdal_shpreadcoords - Single-pass shapefile coordinate reader
   *
   * Consolidates getnfeatures + getglobal_fids + shpinquire + shpgetcoords
   * into ONE function that opens the file once, iterates features once using
   * sequential access, and returns both the point count and coordinates.
   *
   * Two-call protocol from Fortran:
   *   Call 1: count_only=1  => returns localpoints only (coordX/coordY ignored)
   *   Call 2: count_only=0  => fills pre-allocated coordX/coordY arrays
   *
   * This avoids the need for two separate passes or dynamic reallocation
   * while keeping the Fortran allocation pattern simple.
   *=========================================================================*/
...
#define SHPREAD_TIMER_PRINT(pet, count_only, nfeat, npts) \
    if (1) { \
      double _t_now = shpread_gettime(); \
      _t_close = _t_now - _t1; \
      double _elapsed = _t_now - _t_total; \
      printf("[SHP_TIMING] PET %d  pass=%s  features=%lld  points=%d  " \
             "open=%.4f seek=%.4f loop=%.4f close=%.4f  TOTAL=%.4f sec\n", \
             pet, (count_only ? "COUNT" : "READ "), \
             (long long)(nfeat), npts, \
...
#define ESMC_METHOD "c_esmc_gdal_shpreadcoords()"
    ...
    *totaldims = 2;
    ...
    // Skip features before our range using SetNextByIndex for efficiency
    if (my_start > 0) {
      if (OGR_L_SetNextByIndex(hLayer, my_start) != OGRERR_NONE) {
        // Fallback: skip manually
      ...
      if (geomType == wkbPoint) {
        ...
        if (!do_count_only) {
          for (int i = nPts - 1; i >= 0; i--) {
            coordX[point_count + (nPts - 1 - i)] =
                OGR_G_GetX(hGeom, i) * ESMC_CoordSys_Deg2Rad;
            coordY[point_count + (nPts - 1 - i)] =
        ...
        if (!do_count_only) {
          for (int i = 0; i < nPts; i++) {
            coordX[point_count + i] =
                OGR_G_GetX(hGeom, i) * ESMC_CoordSys_Deg2Rad;
            coordY[point_count + i] =
                OGR_G_GetY(hGeom, i) * ESMC_CoordSys_Deg2Rad;
          }
        }
        point_count += nPts;
    ...
    *localpoints = point_count;
    ...
    SHPREAD_TIMER_MARK(_t1)
    GDALClose(hDS);
    SHPREAD_TIMER_PRINT(*local_pet, do_count_only, my_feature_count, point_count)
#endif
