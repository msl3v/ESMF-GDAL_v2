...
#define ESMF_METHOD "ESMF_LocStreamGetConnectivity"
  !BOP
  ! !IROUTINE: ESMF_LocStreamGetConnectivity - Get connectivity information
  !
  ! !INTERFACE:
  subroutine ESMF_LocStreamGetConnectivity(locstream, keywordEnforcer, &
       hasConnectivity, nedges, rc)
    !
    ! !ARGUMENTS:
    type(ESMF_LocStream), intent(in) :: locstream
    ...
    !
    ! !DESCRIPTION:
    !   Query whether LocStream has network connectivity and get basic info.
    !
    ! The arguments are:
    ! \begin{description}
    ! \item[locstream]
...
#define ESMF_METHOD "ESMF_LocStreamCreateFromShapefilePETSc"
  ...
  !
  ! !INTERFACE:
  function ESMF_LocStreamCreateFromShapefilePETSc(filename, &
...
!#include <petsc/petsc.h>
    ...
    integer(c_int), pointer :: nodedist(:), xadj(:), adjncy(:)
    ...
    call PetscInitialized(PETSC_TRUE, ierr)
    ...
    do i = 0, nnodes-1
      do j = xadj(i+1), xadj(i+2)-1
        ...
        edge_list(edge_local*2 + 2) = adjncy(j)
    ...
    if (rank == 0) then
      ...
      print *, '  Nodes with ghosts (rank 0): ', localcount_with_ghosts
    endif
    ...
    !---------------------------------------------------------------------------
    ! Store connectivity in LocStream
    !---------------------------------------------------------------------------
    lstypep => locstream%lstypep
    lstypep%has_connectivity = .true.
