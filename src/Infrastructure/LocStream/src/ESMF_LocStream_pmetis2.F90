...
#define ESMF_METHOD "ESMF_LocStreamCreateFromFile"
  ...
  function ESMF_LocStreamCreateFromFile(filename, keywordEnforcer, &
    ...
    elseif (localfileformat == ESMF_FILEFORMAT_SHAPEFILE) then
       ...
       ! Read shapefile using GDAL - simple coordinate extraction only
       ! For connectivity/network topology, use ESMF_LocStreamCreateFromShapefile()
       
       ! Get total feature count and distribute across PETs
       call c_esmc_gdal_getnfeatures(filename, PetNo, PetCnt, &
            totalfeatures, locfeatures, minid, maxid, localrc)
       if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
            ESMF_CONTEXT, rcToReturn=rc)) return
       
       ! Allocate array to hold global feature IDs
       allocate(gFIDs(totalfeatures))
       
       ! Get global feature IDs (needed for distributed reading)
       call c_esmc_gdal_getglobal_fids(filename, totalfeatures, gFIDs, localrc)
       if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
            ESMF_CONTEXT, rcToReturn=rc)) return
       
       ! Allocate array for local feature IDs
       allocate(FIDs(locfeatures))
       do i = 1, locfeatures
          FIDs(i) = gFIDs(minid + i - 1)
       enddo
       
       ! Inquire shapefile to get number of points
       call c_esmc_gdal_shpinquire(filename, PetNo, PetCnt, &
            localpoints, totaldims, totalfeatures, gFIDs, locfeatures, FIDs, localrc)
       if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
            ESMF_CONTEXT, rcToReturn=rc)) return
       ...
       totalpoints = totalfeatures  ! Approximation - will be corrected by MPI_Allreduce
       coordSys = ESMF_COORDSYS_SPH_RAD  ! c_esmc_gdal_shpgetcoords returns radians
       ...
       if (localcount > 0) then
          call c_esmc_gdal_shpgetcoords(filename, PetNo, locfeatures, &
               FIDs, localcount, coordX, coordY, localrc)
          if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
               ESMF_CONTEXT, rcToReturn=rc)) return
       endif
