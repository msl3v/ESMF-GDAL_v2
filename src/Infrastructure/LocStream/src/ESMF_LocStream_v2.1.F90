...
#ifdef SHPTIMING
       shpt_total = MPI_Wtime() - shpt_total
       write(*, '(A,I6,A,I10,A)') &
            '[SHP_TIMING] PET ', PetNo, '  shapefile_block  localpoints=', &
            localpoints, '  (seconds):'
       write(*, '(A,I6,A,F10.4)') &
            '[SHP_TIMING] PET ', PetNo, '    count_pass   = ', shpt_count
       write(*, '(A,I6,A,F10.4)') &
            '[SHP_TIMING] PET ', PetNo, '    alloc        = ', shpt_alloc
       write(*, '(A,I6,A,F10.4)') &
            '[SHP_TIMING] PET ', PetNo, '    read_pass    = ', shpt_read
       write(*, '(A,I6,A,F10.4)') &
            '[SHP_TIMING] PET ', PetNo, '    SUBTOTAL     = ', shpt_total
       call flush(6)
#endif
...
#ifdef SHPTIMING
    if (localfileformat == ESMF_FILEFORMAT_SHAPEFILE) shpt_t0 = MPI_Wtime()
#endif
...
!    print *, "here 1", PetNo, localrc, starti, localpoints, coordX(1), coordY(1)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
         ESMF_CONTEXT, rcToReturn=rc)) return
#ifdef SHPTIMING
    if (localfileformat == ESMF_FILEFORMAT_SHAPEFILE) &
         shpt_create = MPI_Wtime() - shpt_t0
#endif
!    print *, "here 2", rc, localrc
    ...
    if (localfileformat == ESMF_FILEFORMAT_SHAPEFILE) then
       write(*, '(A,I6,A,F10.4)') &
            '[SHP_TIMING] PET ', PetNo, '    LocStrCreate = ', shpt_create
       write(*, '(A,I6,A,F10.4)') &
            '[SHP_TIMING] PET ', PetNo, '    AddKey(Lon)  = ', shpt_addlon
       write(*, '(A,I6,A,F10.4)') &
            '[SHP_TIMING] PET ', PetNo, '    AddKey(Lat)  = ', shpt_addlat
       shpt_total = shpt_count + shpt_alloc + shpt_read &
                  + shpt_create + shpt_addlon + shpt_addlat
       write(*, '(A,I6,A,F10.4)') &
            '[SHP_TIMING] PET ', PetNo, '    TOTAL        = ', shpt_total
       call flush(6)
    endif
#endif
