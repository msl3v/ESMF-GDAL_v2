...
!  use petsc
  ...
  use petscdmnetwork
  use petscdmda
  use iso_c_binding, only: c_ptr, c_int64_t, c_double, c_char
...
!#endif
  ...
  type ESMF_LocStreamType
     ...
     ! ====== MSL: Network connectivity support ======
     logical                              :: has_connectivity  ! Does this LocStream have network edges?
     integer                              :: nedges            ! Number of edges
     integer, pointer                     :: edge_src(:)       ! Source node IDs
     integer, pointer                     :: edge_dst(:)       ! Destination node IDs
     real(ESMF_KIND_R8), pointer          :: edge_length(:)    ! Edge lengths
     ...
     ! ====== Ghost node support ======
     integer                              :: nowned            ! Number of owned (non-ghost) nodes
     integer                              :: nghost            ! Number of ghost nodes
     integer, pointer                     :: ghost_node_ids(:) ! Global IDs of ghost nodes
     integer, pointer                     :: ghost_owners(:)   ! Owner ranks of ghost nodes
     ! ===============================================
     ESMF_INIT_DECLARE
  ...
  interface ESMF_LocStreamCreate
     ...
     module procedure ESMF_LocStreamCreateFromShapefilePETSc

     ! !DESCRIPTION: 
     ! This interface provides a single entry point for the various 
     !  types of {\tt ESMF\_LocStreamCreate} functions.   
     !EOPI 
  end interface ESMF_LocStreamCreate
  ...
  interface ESMF_LocStreamGetKey
     ...
     !EOPI 
  end interface ESMF_LocStreamGetKey
  ...
  interface ESMF_LocStreamAddKey
     ...
     !EOPI 
  end interface ESMF_LocStreamAddKey
...
!>>          allocate(lFIDs(locFeatures)) ! This is local to this PET
...
!>>       print *, "ShpInquire: ", localpoints, nfeatures, petNo, petCnt, lFIDs(1),lFIDs(locfeatures)
       ...
       ! Set up the locstream info here. No need for setup below. 
       ! Below, we just copy into into the structures needed by ESMF_LocStreamCreate(), etc.
...
#define ESMF_METHOD "ESMF_LocStreamDestruct"
  ...
  subroutine ESMF_LocStreamDestruct(lstypep,rc)
    ...
    ! Deallocate connectivity arrays if present
    if (lstypep%has_connectivity) then
       if (associated(lstypep%edge_src)) deallocate(lstypep%edge_src)
       if (associated(lstypep%edge_dst)) deallocate(lstypep%edge_dst)
       if (associated(lstypep%edge_length)) deallocate(lstypep%edge_length)
       if (associated(lstypep%adj_head)) deallocate(lstypep%adj_head)
       if (associated(lstypep%adj_next)) deallocate(lstypep%adj_next)
    end if
...
#define ESMF_METHOD "ESMF_LocStreamIsCreated()"
  ...
  end function ESMF_LocStreamIsCreated
  !------------------------------------------------------------------------------
...
#define ESMF_METHOD "ESMF_LocStreamCreateSubset"
  ...
  end function ESMF_LocStreamCreateSubset
  !------------------------------------------------------------------------------
...
#define ESMF_METHOD "ESMF_LocStreamCreateMerge"
  ...
  end function ESMF_LocStreamCreateMerge
  !------------------------------------------------------------------------------
...
#define ESMF_METHOD "ESMF_LocStreamCreateByKeys"
  ...
  subroutine ExtractPolylineConnectivity(locstream, filename, &
    ...
    ! This is O(n^2) but necessary to detect junctions where polylines meet
    do i = 1, node_count - 1
...
#define ESMF_METHOD "BuildAdjacencyLists"
  !BOPI
  ! !IROUTINE: BuildAdjacencyLists - Build adjacency lists for efficient traversal
  !
  ! !INTERFACE:
  subroutine BuildAdjacencyLists(lstypep, nnodes, rc)
    !
    ! !ARGUMENTS:
    type(ESMF_LocStreamType), pointer, intent(inout) :: lstypep
    integer, intent(in) :: nnodes
    integer, intent(out) :: rc
    !
    ! !DESCRIPTION:
    !   Builds adjacency list representation (head/next arrays) for
    !   efficient graph traversal in routing algorithms.
    !
    !EOPI
    !------------------------------------------------------------------------------
    integer :: i, u
    ...
    allocate(lstypep%adj_next(lstypep%nedges))
  ...
  end subroutine BuildAdjacencyLists

  !------------------------------------------------------------------------------
#undef ESMF_METHOD
#define ESMF_METHOD "ESMF_LocStreamGetConnectivity"
  !BOP
  ! !IROUTINE: ESMF_LocStreamGetConnectivity - Get network connectivity information
  !
  ! !INTERFACE:
  subroutine ESMF_LocStreamGetConnectivity(locstream, keywordEnforcer, &
       hasConnectivity, nedges, edgeSrc, edgeDst, edgeLength, rc)
    !
    ! !ARGUMENTS:
    type(ESMF_LocStream), intent(in) :: locstream
    type(ESMF_KeywordEnforcer), optional:: keywordEnforcer
    logical, intent(out), optional :: hasConnectivity
    integer, intent(out), optional :: nedges
    integer, pointer, intent(out), optional :: edgeSrc(:)
    integer, pointer, intent(out), optional :: edgeDst(:)
    real(ESMF_KIND_R8), pointer, intent(out), optional :: edgeLength(:)
    integer, intent(out), optional :: rc
    !
    ! !DESCRIPTION:
    !   Query whether LocStream has network connectivity and get connectivity data.
    !   Returns pointers to internal edge arrays. Do not deallocate these pointers.
    !
    ! The arguments are:
    ! \begin{description}
    ! \item[locstream]
    !   The {\tt ESMF\_LocStream} object to query.
    ! \item[{[hasConnectivity]}]
    !   Returns .TRUE. if connectivity information is present.
    ! \item[{[nedges]}]
    !   Number of edges in the network.
    ! \item[{[edgeSrc]}]
    !   Pointer to source node IDs for each edge. Do not deallocate.
    ! \item[{[edgeDst]}]
    !   Pointer to destination node IDs for each edge. Do not deallocate.
    ! \item[{[edgeLength]}]
    !   Pointer to edge lengths/costs. Do not deallocate.
    ! \item[{[rc]}]
    !   Return code; equals {\tt ESMF\_SUCCESS} if there are no errors.
    ! \end{description}
    !
    ...
    ! Return pointers to edge arrays (do not deallocate these!)
    if (present(edgeSrc)) then
       if (associated(lstypep%edge_src)) then
          edgeSrc => lstypep%edge_src
       else
          nullify(edgeSrc)
       endif
    endif

    if (present(edgeDst)) then
       if (associated(lstypep%edge_dst)) then
          edgeDst => lstypep%edge_dst
       else
          nullify(edgeDst)
       endif
    endif

    if (present(edgeLength)) then
       if (associated(lstypep%edge_length)) then
          edgeLength => lstypep%edge_length
       else
          nullify(edgeLength)
       endif
    endif
  ...
  end subroutine ESMF_LocStreamGetConnectivity

  !------------------------------------------------------------------------------
#undef ESMF_METHOD
#define ESMF_METHOD "ESMF_LocStreamGetEdge"
  !BOP
  ! !IROUTINE: ESMF_LocStreamGetEdge - Get information about an edge
  !
  ! !INTERFACE:
  subroutine ESMF_LocStreamGetEdge(locstream, edgeId, keywordEnforcer, &
       srcNode, dstNode, length, rc)
    !
    ! !ARGUMENTS:
    type(ESMF_LocStream), intent(in) :: locstream
    integer, intent(in) :: edgeId
    type(ESMF_KeywordEnforcer), optional:: keywordEnforcer
    integer, intent(out), optional :: srcNode, dstNode
    real(ESMF_KIND_R8), intent(out), optional :: length
    integer, intent(out), optional :: rc
    !
    ! !DESCRIPTION:
    !   Get information about a specific edge in the network.
    !
    ! The arguments are:
    ! \begin{description}
    ! \item[locstream]
    !   The {\tt ESMF\_LocStream} object.
    ! \item[edgeId]
    !   The edge identifier (1 to nedges).
    ! \item[{[srcNode]}]
    !   Source node ID of the edge.
    ! \item[{[dstNode]}]
    !   Destination node ID of the edge.
    ! \item[{[length]}]
    !   Length of the edge (computed from coordinates).
    ! \item[{[rc]}]
    !   Return code; equals {\tt ESMF\_SUCCESS} if there are no errors.
    ! \end{description}
    !
  ...
  end subroutine ESMF_LocStreamGetEdge

  !------------------------------------------------------------------------------
#undef ESMF_METHOD
#define ESMF_METHOD "ESMF_LocStreamGetNeighbors"
  !BOP
  ! !IROUTINE: ESMF_LocStreamGetNeighbors - Get neighboring nodes
  !
  ! !INTERFACE:
  subroutine ESMF_LocStreamGetNeighbors(locstream, nodeId, keywordEnforcer, &
       neighbors, edgeIds, distances, nNeighbors, rc)
    !
    ! !ARGUMENTS:
    type(ESMF_LocStream), intent(in) :: locstream
    integer, intent(in) :: nodeId
    type(ESMF_KeywordEnforcer), optional:: keywordEnforcer
    integer, allocatable, intent(out), optional :: neighbors(:)
    integer, allocatable, intent(out), optional :: edgeIds(:)
    real(ESMF_KIND_R8), allocatable, intent(out), optional :: distances(:)
    integer, intent(out), optional :: nNeighbors
    integer, intent(out), optional :: rc
    !
    ! !DESCRIPTION:
    !   Get all neighboring nodes connected to the given node via edges.
    !   Useful for graph traversal and routing algorithms.
    !
    ! The arguments are:
    ! \begin{description}
    ! \item[locstream]
    ...
    ! \item[{[neighbors]}]
    !   Array of neighboring node IDs.
    ! \item[{[edgeIds]}]
    !   Array of edge IDs connecting to neighbors.
    ! \item[{[distances]}]
    !   Array of distances to each neighbor.
    ! \item[{[nNeighbors]}]
    ...
    allocate(temp_neighbors(count))
    allocate(temp_edges(count))
    allocate(temp_distances(count))
    ...
    if (present(neighbors)) then
       allocate(neighbors(count))
       neighbors = temp_neighbors
    end if

    if (present(edgeIds)) then
       allocate(edgeIds(count))
       edgeIds = temp_edges
    end if
  ...
  end subroutine ESMF_LocStreamGetNeighbors

  !------------------------------------------------------------------------------
#undef ESMF_METHOD
#define ESMF_METHOD "ESMF_LocStreamGetBoundaryFlags"
  !BOP
  ! !IROUTINE: ESMF_LocStreamGetBoundaryFlags - Get boundary node flags
  !
  ! !INTERFACE:
  subroutine ESMF_LocStreamGetBoundaryFlags(locstream, keywordEnforcer, &
       boundaryFlags, nBoundary, rc)
    !
    ! !ARGUMENTS:
    type(ESMF_LocStream), intent(in) :: locstream
    type(ESMF_KeywordEnforcer), optional:: keywordEnforcer ! < ESMF_KeywordEnforcer >
    logical, pointer, intent(out), optional :: boundaryFlags(:)
    integer, intent(out), optional :: nBoundary
    integer, intent(out), optional :: rc
    !
    ! !DESCRIPTION:
    !   Get boundary node flags from LocStream. Boundary nodes are endpoints
    !   (1 edge) or junctions (3+ edges). Internal nodes (2 edges) are not boundaries.
    !
    ! The arguments are:
    ! \begin{description}
    ! \item[locstream]
    !   The {\tt ESMF\_LocStream} object.
    ! \item[{[boundaryFlags]}]
    !   Pointer to boundary flag array. Do not deallocate.
    ! \item[{[nBoundary]}]
    ...
    if (present(boundaryFlags)) then
       boundaryFlags => lstypep%is_boundary
    end if
  ...
  end subroutine ESMF_LocStreamGetBoundaryFlags

  !------------------------------------------------------------------------------
#undef ESMF_METHOD
#define ESMF_METHOD "ESMF_LocStreamAddGhostEdges"
  !BOP
  ! !IROUTINE: ESMF_LocStreamAddGhostEdges - Add ghost edges to connectivity
  !
  ! !INTERFACE:
  subroutine ESMF_LocStreamAddGhostEdges(locstream, keywordEnforcer, &
       ghostSrc, ghostDst, ghostLength, nGhosts, rc)
    !
    ! !ARGUMENTS:
    type(ESMF_LocStream), intent(inout) :: locstream
    type(ESMF_KeywordEnforcer), optional:: keywordEnforcer ! < ESMF_KeywordEnforcer >
    integer, intent(in) :: ghostSrc(:)
    integer, intent(in) :: ghostDst(:)
    real(ESMF_KIND_R8), intent(in), optional :: ghostLength(:)
    integer, intent(in) :: nGhosts
    integer, intent(out), optional :: rc
    !
    ! !DESCRIPTION:
    !   Add ghost edges to LocStream connectivity for cross-PE connections.
    !
    ! The arguments are:
    ! \begin{description}
    ! \item[locstream]
    !   The {\tt ESMF\_LocStream} object to modify.
    ! \item[ghostSrc]
    !   Source node IDs for ghost edges.
    ! \item[ghostDst]
    !   Destination node IDs for ghost edges (may be global IDs).
    ! \item[{[ghostLength]}]
    !   Edge lengths. If not provided, defaults to 0.
    ! \item[nGhosts]
    !   Number of ghost edges to add.
    ! \item[{[rc]}]
    !   Return code; equals {\tt ESMF\_SUCCESS} if there are no errors.
    ! \end{description}
    !
    ...
    allocate(new_edge_dst(new_nedges))
    allocate(new_edge_length(new_nedges))

    ! Copy existing edges
    new_edge_src(1:old_nedges) = lstypep%edge_src(1:old_nedges)
    new_edge_dst(1:old_nedges) = lstypep%edge_dst(1:old_nedges)
    new_edge_length(1:old_nedges) = lstypep%edge_length(1:old_nedges)

    ! Add ghost edges
    do i = 1, nGhosts
       new_edge_src(old_nedges + i) = ghostSrc(i)
       new_edge_dst(old_nedges + i) = ghostDst(i)
       if (present(ghostLength)) then
          new_edge_length(old_nedges + i) = ghostLength(i)
       else
          new_edge_length(old_nedges + i) = 0.0  ! Default to zero (same location)
       end if
    end do

    ! Replace edge arrays
    deallocate(lstypep%edge_src, lstypep%edge_dst, lstypep%edge_length)
    allocate(lstypep%edge_src(new_nedges))
    allocate(lstypep%edge_dst(new_nedges))
    allocate(lstypep%edge_length(new_nedges))

    lstypep%edge_src = new_edge_src
    lstypep%edge_dst = new_edge_dst
    lstypep%edge_length = new_edge_length
    lstypep%nedges = new_nedges
...
!#ifdef ESMF_PETSC
  !-----------------------------------------------------------------------------
#undef ESMF_METHOD
#define ESMF_METHOD "ESMF_LocStreamCreateFromShapefilePETSc"
  !BOP
  ! !IROUTINE: ESMF_LocStreamCreateFromShapefilePETSc
  !
  ! !INTERFACE:
  function ESMF_LocStreamCreateFromShapefilePETSc(filename, keywordEnforcer, &
       tolerance, name, usePETSc, rc) result(locstream)
    ...
    character(len=*), intent(in), optional :: name
    logical, intent(in) :: usePETSc  ! NEW: distinguishing argument
    integer, intent(out), optional :: rc
    !
    ! !DESCRIPTION:
    !   Creates an ESMF LocStream from a shapefile using PETSc DMNetwork
    !   for automatic ghost node management and inter-PE communication.
    !
    !   This method:
    !   1. Reads shapefile using ParMETIS graph builder
    !   2. Creates PETSc DMNetwork from the graph
    !   3. PETSc automatically identifies and creates ghost nodes
    !   4. Converts DMNetwork to ESMF LocStream with ghosts
    !
    ! The arguments are:
    ! \begin{description}
    ! \item[filename]
    !   Path to shapefile
    ! \item[{[tolerance]}]
    !   Coordinate tolerance for node merging (default: 1e-6)
    ! \item[{[name]}]
    !   Name for the LocStream
    ! \item[{[rc]}]
    !   Return code; equals {\tt ESMF\_SUCCESS} if there are no errors.
    ! \end{description}
    !
...
!#include <petsc/petsc.h>
    ! Local variables
    integer :: localrc, rank, size, i, j
    type(ESMF_VM) :: vm
    real(ESMF_KIND_R8) :: tol
    integer :: mpi_comm_int
    integer :: ierr
    
    ! Shapefile graph data from C
    ...
    ! PETSc DMNetwork variables
    DM :: dm_network, dm_plex
    PetscInt :: nsubnet, nvertex, nedge_local
    PetscInt :: vstart, vend, estart, eend
    PetscInt :: vertex_local, edge_local
    PetscInt, allocatable :: edge_list(:)
    PetscSection :: section
    PetscInt :: pstart, pend
    Vec :: coords_global, coords_local
    PetscScalar, pointer :: coord_array(:)
    PetscBool :: petsc_is_init
    
    ! ESMF variables
    type(ESMF_DistGrid) :: distgrid
    real(ESMF_KIND_R8), pointer :: coordLon(:), coordLat(:)
    integer :: localcount_with_ghosts, total_nodes
    integer :: nowned_nodes, nghost_nodes
    integer, allocatable :: ghost_node_global_ids(:), ghost_node_owners(:)
    integer :: ghost_local_idx, ghost_global_id
    type(ESMF_LocStreamType), pointer :: lstypep
    
    integer :: source_rank, source_nnodes, ghost_owner, ghost_local_on_owner
    real(c_double), allocatable :: temp_coords(:)
 
   ! C interface declaration
    interface
      subroutine shapefile_to_parmetis_graph_f(filename, comm, &
           nodedist, xadj, adjncy, node_x, node_y, nnodes, nedges, &
           tolerance, ierr) bind(C, name="shapefile_to_parmetis_graph_f")
        use iso_c_binding
      ...
      subroutine free_parmetis_graph_f(nodedist, xadj, adjncy, &
           node_x, node_y) bind(C, name="free_parmetis_graph_f")
        use iso_c_binding
        type(c_ptr), value, intent(in) :: nodedist, xadj, adjncy
        type(c_ptr), value, intent(in) :: node_x, node_y
      end subroutine
    end interface
    ...
    !---------------------------------------------------------------------------
    ! Get MPI information from ESMF
    !---------------------------------------------------------------------------
    call ESMF_VMGetCurrent(vm, rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
         ESMF_CONTEXT, rcToReturn=rc)) return
    
    call ESMF_VMGet(vm, localPet=rank, petCount=size, &
                    mpiCommunicator=mpi_comm_int, rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
         ESMF_CONTEXT, rcToReturn=rc)) return
    ...
    !---------------------------------------------------------------------------
    ! Read shapefile and build graph using ParMETIS builder
    !---------------------------------------------------------------------------
    if (rank == 0) then
      print *, '========================================='
      print *, 'Creating LocStream from shapefile with PETSc'
    ...
    call shapefile_to_parmetis_graph_f(c_filename, mpi_comm_int, &
         nodedist_ptr, xadj_ptr, adjncy_ptr, node_x_ptr, node_y_ptr, &
         nnodes, nedges, tol, c_ierr)
    
    if (c_ierr /= 0) then
      call ESMF_LogSetError(ESMF_FAILURE, &
           msg="Failed to read shapefile", &
           ESMF_CONTEXT, rcToReturn=rc)
      return
    endif
    
    ! Convert C pointers to Fortran
    ...
    total_nodes = int(nodedist(size+1))
    
    print *, 'DEBUG Rank ', rank, ': nnodes=', nnodes, ' nedges=', nedges
    print *, 'DEBUG Rank ', rank, ': nodedist(', rank+1, ')=', nodedist(rank+1), &
             ' nodedist(', rank+2, ')=', nodedist(rank+2)
    ...
    !---------------------------------------------------------------------------
    ! Create PETSc DMNetwork from graph data
    !---------------------------------------------------------------------------
    
    ! Initialize PETSc if not already done
    call PetscInitialized(petsc_is_init, ierr)
    if (petsc_is_init .neqv. PETSC_TRUE) then
      call PetscInitialize(ierr)
      CHKERRQ(ierr)
    endif
    
    ! Create DMNetwork
    call DMNetworkCreate(PETSC_COMM_WORLD, dm_network, ierr)
    CHKERRQ(ierr)
    
    ! Set up network topology
    nsubnet = 1  ! Single subnetwork
    nvertex = nnodes
    nedge_local = nedges
    
    print *, 'DEBUG Rank ', rank, ': Building edge_list with ', nedge_local, ' edges'
    print *, 'DEBUG Rank ', rank, ': nodedist(', rank+1, ')=', nodedist(rank+1)
    
    ! Build edge list from CSR format (xadj, adjncy)
    allocate(edge_list(nedge_local * 2))
    edge_local = 0
    do i = 0, nnodes-1
      do j = int(xadj(i+1)), int(xadj(i+2))-1
        edge_list(edge_local*2 + 1) = nodedist(rank+1) + i
        edge_list(edge_local*2 + 2) = adjncy(j+1)
        edge_local = edge_local + 1
      enddo
    enddo
    
    print *, 'DEBUG Rank ', rank, ': Built ', edge_local, ' edges (expected ', nedge_local, ')'
    if (nedge_local > 0) then
      print *, 'DEBUG Rank ', rank, ': First edge: ', edge_list(1), ' -> ', edge_list(2)
    endif
    
    ! Register network components
    call DMNetworkSetNumSubNetworks(dm_network, PETSC_DECIDE, nsubnet, ierr)
    CHKERRQ(ierr)
    
    call DMNetworkAddSubnetwork(dm_network, "", nedge_local, &
                                edge_list, PETSC_NULL_INTEGER, ierr)
    CHKERRQ(ierr)
    
    ! Layout the network (this identifies ghost nodes!)
    call DMNetworkLayoutSetUp(dm_network, ierr)
    CHKERRQ(ierr)
    
    ! Finalize setup - creates ghost nodes automatically
    call DMSetUp(dm_network, ierr)
    CHKERRQ(ierr)
    
    deallocate(edge_list)
    
    !---------------------------------------------------------------------------
    ! Get node ranges (includes ghosts)
    !---------------------------------------------------------------------------
    call DMNetworkGetVertexRange(dm_network, vstart, vend, ierr)
    CHKERRQ(ierr)
    
    localcount_with_ghosts = int(vend - vstart)
    
    ! Get owned node count (without ghosts)
    ! The first nnodes are owned, rest are ghosts
    nowned_nodes = int(nnodes)
    nghost_nodes = localcount_with_ghosts - nowned_nodes
    
    print *, 'DEBUG Rank ', rank, ': vstart=', vstart, ' vend=', vend
    print *, 'DEBUG Rank ', rank, ': Owned=', nowned_nodes, ' Ghosts=', nghost_nodes, ' Total=', localcount_with_ghosts
    
    if (rank == 0) then
      print *, 'PETSc DMNetwork created:'
      print *, '  Owned nodes (rank 0): ', nowned_nodes
      print *, '  Ghost nodes (rank 0): ', nghost_nodes
      print *, '  Total with ghosts: ', localcount_with_ghosts
    endif
    
    ! Allocate arrays to track ghost node information
    allocate(ghost_node_global_ids(nghost_nodes))
    allocate(ghost_node_owners(nghost_nodes))
    
    ! Identify ghost nodes and their owners
    ! Ghost nodes are those with global IDs outside our owned range
    do i = 1, nghost_nodes
      ghost_local_idx = nowned_nodes + i
      ! Get global ID from DMNetwork
      call DMNetworkGetGlobalVertexIndex(dm_network, ghost_local_idx - 1 + vstart, &
                                         ghost_global_id, ierr)
      CHKERRQ(ierr)
      
      ghost_node_global_ids(i) = int(ghost_global_id)
      
      ! Determine owner rank from nodedist
      do j = 0, size-1
        if (ghost_global_id >= nodedist(j+1) .and. &
            ghost_global_id < nodedist(j+2)) then
          ghost_node_owners(i) = j
          exit
        endif
      enddo
    enddo
    ...
    !---------------------------------------------------------------------------
    ! Create coordinate vector in PETSc
    !---------------------------------------------------------------------------
    call DMNetworkGetPlex(dm_network, dm_plex, ierr)
    CHKERRQ(ierr)
    
    call DMGetGlobalVector(dm_network, coords_global, ierr)
    CHKERRQ(ierr)
    
    call DMGetLocalVector(dm_network, coords_local, ierr)
    CHKERRQ(ierr)
    
    ! Fill coordinates for owned nodes
    call VecGetArrayF90(coords_global, coord_array, ierr)
    CHKERRQ(ierr)
    
    do i = 1, nnodes
      coord_array((i-1)*2 + 1) = node_x(i)  ! X/Lon
      coord_array((i-1)*2 + 2) = node_y(i)  ! Y/Lat
    enddo
    
    call VecRestoreArrayF90(coords_global, coord_array, ierr)
    CHKERRQ(ierr)
    
    ! Update ghost coordinates
    call DMGlobalToLocalBegin(dm_network, coords_global, INSERT_VALUES, &
                              coords_local, ierr)
    CHKERRQ(ierr)
    call DMGlobalToLocalEnd(dm_network, coords_global, INSERT_VALUES, &
                            coords_local, ierr)
    CHKERRQ(ierr)
    
    !---------------------------------------------------------------------------
    ! Create ESMF LocStream from DMNetwork
    !---------------------------------------------------------------------------
    
    ! Create DistGrid (using total nodes, not including ghosts in dist)
    distgrid = ESMF_DistGridCreate(minIndex=(/1/), &
                                   maxIndex=(/total_nodes/), &
                                   regDecomp=(/size/), &
                                   rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
         ESMF_CONTEXT, rcToReturn=rc)) return
    ...
    ! Create LocStream
    if (present(name)) then
      locstream = ESMF_LocStreamCreate(distgrid=distgrid, name=name, rc=localrc)
    else
      locstream = ESMF_LocStreamCreate(distgrid=distgrid, &
                                      name="ShapefilePETSc", rc=localrc)
    endif
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
         ESMF_CONTEXT, rcToReturn=rc)) return
    
    ! Add coordinate keys (including space for ghosts)
    call ESMF_LocStreamAddKey(locstream, keyName="ESMF:Lon", &
                               keyTypeKind=ESMF_TYPEKIND_R8, &
                               keyUnits="degrees_east", rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
         ESMF_CONTEXT, rcToReturn=rc)) return
    
    call ESMF_LocStreamAddKey(locstream, keyName="ESMF:Lat", &
                               keyTypeKind=ESMF_TYPEKIND_R8, &
                               keyUnits="degrees_north", rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
         ESMF_CONTEXT, rcToReturn=rc)) return
    
    ! Get coordinate arrays
    call ESMF_LocStreamGetKey(locstream, keyName="ESMF:Lon", &
                               farray=coordLon, rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
         ESMF_CONTEXT, rcToReturn=rc)) return
    
    call ESMF_LocStreamGetKey(locstream, keyName="ESMF:Lat", &
                               farray=coordLat, rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
         ESMF_CONTEXT, rcToReturn=rc)) return
    
    ! Copy coordinates from PETSc (includes ghosts)
    call VecGetArrayReadF90(coords_local, coord_array, ierr)
    CHKERRQ(ierr)
    
    do i = 1, localcount_with_ghosts
      coordLon(i) = coord_array((i-1)*2 + 1)
      coordLat(i) = coord_array((i-1)*2 + 2)
    enddo
    
    call VecRestoreArrayReadF90(coords_local, coord_array, ierr)
    CHKERRQ(ierr)
    
    !---------------------------------------------------------------------------
    ! Store connectivity and ghost information in LocStream
    !---------------------------------------------------------------------------
    lstypep => locstream%lstypep
    lstypep%has_connectivity = .true.
    ...
    lstypep%nowned = nowned_nodes
    lstypep%nghost = nghost_nodes
    
    if (nghost_nodes > 0) then
      allocate(lstypep%ghost_node_ids(nghost_nodes))
      allocate(lstypep%ghost_owners(nghost_nodes))
      lstypep%ghost_node_ids = ghost_node_global_ids
      lstypep%ghost_owners = ghost_node_owners
    else
      nullify(lstypep%ghost_node_ids)
      nullify(lstypep%ghost_owners)
    endif
    
    ! Allocate and fill edge arrays from CSR format
    ...
    edge_local = 0
    do i = 0, nnodes-1
      do j = xadj(i+1), xadj(i+2)-1
        edge_local = edge_local + 1
        lstypep%edge_src(edge_local) = nodedist(rank+1) + i + 1
        lstypep%edge_dst(edge_local) = adjncy(j) + 1
        lstypep%edge_length(edge_local) = 0.0_ESMF_KIND_R8  ! TODO: compute
      enddo
    enddo
    ...
    ! Cleanup
    !---------------------------------------------------------------------------
    if (allocated(ghost_node_global_ids)) deallocate(ghost_node_global_ids)
    if (allocated(ghost_node_owners)) deallocate(ghost_node_owners)
    
    call DMRestoreGlobalVector(dm_network, coords_global, ierr)
    call DMRestoreLocalVector(dm_network, coords_local, ierr)
    
    ! Keep dm_network alive - could store in LocStream for later use
    ! For now, just destroy it
    call DMDestroy(dm_network, ierr)
    CHKERRQ(ierr)
    ...
    if (rank == 0) then
      print *, '========================================='
      print *, 'LocStream created successfully with PETSc'
      print *, '========================================='
    endif
    
    if (present(rc)) rc = ESMF_SUCCESS
    
  end function ESMF_LocStreamCreateFromShapefilePETSc

!#endif  /* ESMF_PETSC */

  !-----------------------------------------------------------------------------
#undef ESMF_METHOD
#define ESMF_METHOD "ESMF_LocStreamGetGhostInfo"
  !BOP
  ! !IROUTINE: ESMF_LocStreamGetGhostInfo - Get ghost node information
  !
  ! !INTERFACE:
  subroutine ESMF_LocStreamGetGhostInfo(locstream, keywordEnforcer, &
       nowned, nghost, ghostNodeIds, ghostOwners, rc)
    !
    ! !ARGUMENTS:
    type(ESMF_LocStream), intent(in) :: locstream
    type(ESMF_KeywordEnforcer), optional:: keywordEnforcer
    integer, intent(out), optional :: nowned
    integer, intent(out), optional :: nghost
    integer, pointer, intent(out), optional :: ghostNodeIds(:)
    integer, pointer, intent(out), optional :: ghostOwners(:)
    integer, intent(out), optional :: rc
    !
    ! !DESCRIPTION:
    !   Get ghost node information from LocStream created with PETSc.
    !   Ghost nodes are nodes owned by other ranks but needed locally
    ...
    !   The {\tt ESMF\_LocStream} object.
    ! \item[{[nowned]}]
    !   Number of owned (non-ghost) nodes on this rank.
    ! \item[{[nghost]}]
    !   Number of ghost nodes on this rank.
    ! \item[{[ghostNodeIds]}]
    !   Pointer to array of global node IDs for ghost nodes. Do not deallocate.
    ! \item[{[ghostOwners]}]
    !   Pointer to array of owner ranks for ghost nodes. Do not deallocate.
    ! \item[{[rc]}]
    !   Return code; equals {\tt ESMF\_SUCCESS} if there are no errors.
    ! \end{description}
    !
    ...
    if (present(nowned)) then
      nowned = lstypep%nowned
    endif

    if (present(nghost)) then
      nghost = lstypep%nghost
    endif

    if (present(ghostNodeIds)) then
      if (associated(lstypep%ghost_node_ids)) then
        ghostNodeIds => lstypep%ghost_node_ids
      else
        nullify(ghostNodeIds)
      endif
    endif

    if (present(ghostOwners)) then
      if (associated(lstypep%ghost_owners)) then
        ghostOwners => lstypep%ghost_owners
      else
        nullify(ghostOwners)
      endif
    endif
  ...
  end subroutine ESMF_LocStreamGetGhostInfo
