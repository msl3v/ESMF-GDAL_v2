...
#include "ESMF.h"
  ...
  type ESMF_LineStreamType
     ...
     ! --- Segment connectivity (ESMF_Array on segDistgrid) ---
     ! Shape (2, localSegCount): row 1 = start node global ID,
     !                            row 2 = end node global ID.
     type(ESMF_Array)            :: segConnArray
     logical                     :: destroySegConn
...
!EOP
    ...
    ESMF_LineStreamIsCreated = ESMF_INIT_CHECK_SET_CREATED(linestream)
...
!EOP
    ...
    ! -----------------------------------------------------------------
    ! Create segment connectivity Array on segDistgrid
    ! This is a 2D array with undistributed leading dimension of size 2
    ! (start node, end node) and distributed trailing dimension
    ! matching the segment distribution.
    ! -----------------------------------------------------------------
    lstypep%segConnArray = ESMF_ArrayCreate( &
         lstypep%segDistgrid, &
         ESMF_TYPEKIND_I4, &
         indexflag=indexflagLocal, &
         haloSeqIndexList=(/0/), &    ! placeholder, no halo
         name="SegConn", rc=localrc)
    ! The above approach doesn't give us 2D. Instead, store connectivity
    ! as two separate 1D arrays packed into the type, and create the
    ! ESMF_Array as a 1D interleaved array. However, the cleanest approach
    ! for a (2, nSegs) array is to use an undistributed dimension.
    !
    ! Re-create properly: use ArrayCreate with farrayPtr for a 2D array.
    ! ESMF_Array supports undistributed dimensions when created from a
    ! Fortran array pointer with rank > distgrid dimension count.
    call ESMF_ArrayDestroy(lstypep%segConnArray, rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
         ESMF_CONTEXT, rcToReturn=rc)) return

    block
       integer(ESMF_KIND_I4), allocatable, target :: connWork(:,:)
       integer(ESMF_KIND_I4), pointer :: connPtr(:,:)

       ! Pack connectivity into (2, localSegs) work array
       allocate(connWork(2, max(localSegs,1)))
       do i = 1, localSegs
          connWork(1, i) = localSegSrc(i)
          connWork(2, i) = localSegDst(i)
       enddo

       lstypep%segConnArray = ESMF_ArrayCreate( &
            lstypep%segDistgrid, connWork, &
            datacopyflag=ESMF_DATACOPY_VALUE, &
            indexflag=indexflagLocal, &
            name="SegConn", rc=localrc)
       if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
...
!EOP
    ...
    call ESMF_DistGridGet(lstypep%segDistgrid, localDe=lDE, &
         exclusiveLBound=eLB, exclusiveUBound=eUB, rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
         ESMF_CONTEXT, rcToReturn=rc)) return

    if (present(exclusiveLBound)) exclusiveLBound = eLB(1)
    if (present(exclusiveUBound)) exclusiveUBound = eUB(1)
...
!
! !INTERFACE:
  subroutine ESMF_LineStreamGetSegConn(linestream, connArray, rc)
!
! !ARGUMENTS:
    type(ESMF_LineStream), intent(in)  :: linestream
    type(ESMF_Array),      intent(out) :: connArray
    integer,               intent(out), optional :: rc
!
! !DESCRIPTION:
!   Get a reference to the segment connectivity Array. This is a 2D
!   Array of shape (2, localSegCount) where row 1 holds start node
!   global IDs and row 2 holds end node global IDs.
!EOP
    ...
    lstypep => linestream%lstypep
    connArray = lstypep%segConnArray
...
#define ESMF_METHOD "ESMF_LineStreamDestruct"
  subroutine ESMF_LineStreamDestruct(lstypep, rc)
    ...
    if (status .eq. ESMF_STATUS_READY) then
       ...
       ! Destroy segment connectivity Array
       if (lstypep%destroySegConn) then
          call ESMF_ArrayDestroy(lstypep%segConnArray, rc=localrc)
          if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
               ESMF_CONTEXT, rcToReturn=rc)) return
       endif
